...
connect_to();
$sql = "SELECT * FROM `receipt` WHERE `Amount` = '$amountIn' AND `office` = '$dbase' AND `group` = '$group'";

echo( "<html>" );
echo( "<h1>Receipt by Amount</h1>" );
$itemList = mysql_query($sql);

if($itemList == 0)
{
	log_error("Receipt Select Failure: $sql", "rsearch_amount.php", $_SESSION["username"], $_SESSION["group"]);
...
echo( "</table><br>" );
echo( "<form action=\"rsearch_amount.php\" method=\"POST\">" );
echo( "Other receipt: $" );
echo( "<input class=\"blue\" type=\"text\" name=\"amount\">" );
echo( "<input type=\"Button\" value=\"Submit\" onclick=\"javascript:this.value='Please Wait';this.disabled=1;document.forms[0].submit()\"> " );
echo( "</form>" );
echo( "<br>" );
echo( "Search Options: <a href = \"receipt_search.php\">Noncompleted</a> / <a href = \"rsearch_id.php\">ID</a> / <a href = \"rsearch_date.php\">Date</a> / <a href = \"rsearch_name.php\">Name</a> / Amount / <a href=\"rsearch_company.php\">Company</a>" );
?>
