...
echo( "<html>" );
echo( "<h1>Receipt by Last Name</h1>" );

if($lname != "")
{
	connect_to();
	$sql = "SELECT * FROM `receipt` WHERE `LastName` LIKE '%$lname%' AND `office` = '$dbase' AND `group` = '$group'";
...
if($itemList == 0 && $skipItemList != 1)
{
	log_error("Receipt Select Failure: $sql", "rsearch_name.php", $_SESSION["username"], $_SESSION["group"]);
...
echo( "</table><br>" );
echo( "<form action=\"rsearch_name.php\" method=\"POST\">" );
echo( "Other receipt: " );
echo( "<input class=\"blue\" type=\"text\" name=\"lname\">" );
echo( "<input type=\"Button\" value=\"Submit\" onclick=\"javascript:this.value='Please Wait';this.disabled=1;document.forms[0].submit()\"> " );
echo( "</form>" );
echo( "<br>" );
echo( "Search Options: <a href = \"receipt_search.php\">Noncompleted</a> / <a href = \"rsearch_id.php\">ID</a> / <a href = \"rsearch_date.php\">Date</a> / Name / <a href=\"rsearch_amount.php\">Amount</a> / <a href=\"rsearch_company.php\">Company</a>" );
?>
