...
$sql = "SELECT * FROM `companyaddress` WHERE `Number` = '$num'";

$itemList = mysql_query($sql);

if($itemList == 0)
{
	log_error("Company Address Select Failure: $sql2", "companyAddress_edit.php", $_SESSION["username"], $_SESSION["group"]);
	die("Could not obtain record");
}
else
{
	echo( "<h1>Company Address Edit</h1>" );
}

while($row = mysql_fetch_array($itemList))
{
	echo( "<form method=\"post\" action=\"companyAddress_update.php\">" );
